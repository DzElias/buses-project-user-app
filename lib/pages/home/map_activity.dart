...
class _MapActivityState extends State<MapActivity>
    ...
    return Scaffold(
        ...
        body: Stack(
          children: [
            ...
            !startStop ? showTablero(width, buss) : const SizedBox(),
            !startStop
                ...
                children: [
                  ...
                  BlocBuilder<StopsBloc, StopsState>(
                    builder: (context, state) {
                      ...
                      Stop proxStop = state.stops[state.stops
                          .indexWhere((element) => element.id == proxStopID)];

                      return Text(proxStop.esperas.toString(),
                          style: const TextStyle(
                              color: Colors.white,
                              fontSize: 12,
                              fontWeight: FontWeight.w600));
  ...
  Stop getStopById(String id) {
    List<Stop> stopss = Provider.of<StopsBloc>(context, listen: false).state.stops;
  ...
  Widget startTracking(String busID, SocketService socketService) {
    return BlocBuilder<MyLocationBloc, MyLocationState>(
      builder: (context, state) {
        ...
        Stop stop = getStopById(buss.proximaParada);

        if (calculateDistance(position, LatLng(stop.latitud, stop.longitud)) <
            40) {
          int index = buss.paradas.indexWhere((element) => element == stop.id);
          var nextStopId = "";
          if (index == buss.paradas.length) {
            nextStopId = buss.paradas[0];
          } else {
            nextStopId = buss.paradas[index + 1];
          }
          socketService.socket.emit("changeNextStop", [
            [buss.id, nextStopId]
          ]);
          Future.delayed(Duration.zero).then((value) {
            setState(() {
              buss.proximaParada = nextStopId;
              rutaCoords == [];
            });
          });
        }
  ...
  showAlert() {
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Desea finalizar el recorrido?'),
        actions: [
          TextButton(
            onPressed: () {
